...
func TestConvertToLightcurveObjects(t *testing.T) {
	tests := []struct {
		...
		{
			name: "with multiple objects",
			response: []lightCurveResponse{
				{Id: 123, FilterId: 1, FieldId: 2, Nepochs: 1, ObjRa: 3.4, ObjDec: 5.6, Rcid: 7, Hmjd: []float64{8.9}, Mag: []float64{10.1}, Magerr: []float64{0.1}},
				{Id: 456, FilterId: 2, FieldId: 3, Nepochs: 1, ObjRa: 4.5, ObjDec: 6.7, Rcid: 8, Hmjd: []float64{9.2}, Mag: []float64{11.2}, Magerr: []float64{0.2}},
			},
			want: []lightcurve.LightcurveObject{
				Detection{Oid: 123, FilterId: 1, FieldId: 2, Nepochs: 1, ObjRa: 3.4, ObjDec: 5.6, Rcid: 7, Hmjd: 8.9, Mag: 10.1, Magerr: 0.1},
				Detection{Oid: 456, FilterId: 2, FieldId: 3, Nepochs: 1, ObjRa: 4.5, ObjDec: 6.7, Rcid: 8, Hmjd: 9.2, Mag: 11.2, Magerr: 0.2},
			},
		},
		{
...
func TestParseLightcurveResponse(t *testing.T) {
	t.Run("parses array response", func(t *testing.T) {
		responses, err := parseLightcurveResponse([]byte(`[{"_id":1,"hmjd":[1],"mag":[2],"magerr":[0.1]}]`))
		require.NoError(t, err)
		require.Len(t, responses, 1)
		require.Equal(t, int64(1), responses[0].Id)
	})

	t.Run("parses object response", func(t *testing.T) {
		responses, err := parseLightcurveResponse([]byte(`{"_id":1,"hmjd":[1],"mag":[2],"magerr":[0.1]}`))
		require.NoError(t, err)
		require.Len(t, responses, 1)
		require.Equal(t, int64(1), responses[0].Id)
...
func TestFetchLightcurve(t *testing.T) {
	...
	t.Run("returns detections for array response", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			require.Equal(t, "/light_curve/", r.URL.Path)
			_, err := w.Write([]byte(`[{"_id":1,"filterid":2,"fieldid":3,"nepochs":1,"objra":4.5,"objdec":6.7,"rcid":8,"hmjd":[9.1],"mag":[20.1],"magerr":[0.1]},{"_id":2,"filterid":4,"fieldid":5,"nepochs":1,"objra":7.8,"objdec":9.1,"rcid":6,"hmjd":[9.2],"mag":[20.2],"magerr":[0.2]}]`))
			require.NoError(t, err)
		}))
		defer server.Close()
		...
		require.NoError(t, result.Error)
		require.Equal(t, lightcurve.Lightcurve{Detections: []lightcurve.LightcurveObject{
			Detection{Oid: 1, FilterId: 2, FieldId: 3, Nepochs: 1, ObjRa: 4.5, ObjDec: 6.7, Rcid: 8, Hmjd: 9.1, Mag: 20.1, Magerr: 0.1},
			Detection{Oid: 2, FilterId: 4, FieldId: 5, Nepochs: 1, ObjRa: 7.8, ObjDec: 9.1, Rcid: 6, Hmjd: 9.2, Mag: 20.2, Magerr: 0.2},
		}}, result.Lightcurve)
	})
