...
func newAllwiseMetadataService(t *testing.T) (*MetadataService, *repository.Queries, *sql.DB) {
	t.Helper()
	...
	return &MetadataService{resolver: catalog.NewResolver(queries)}, queries, db
}

func insertAllwiseMetadata(t *testing.T, queries *repository.Queries, id string, ra, dec float64) {
	t.Helper()
	ctx := context.Background()
	require.NoError(t, queries.InsertObject(ctx, repository.InsertObjectParams{ID: id, Ipix: 1, Ra: ra, Dec: dec, Cat: "allwise"}))
...
func TestMetadata_FindByID(t *testing.T) {
	m, queries, _ := newAllwiseMetadataService(t)
...
func TestMetadata_Bulk_EmptyResult(t *testing.T) {
	m, _, _ := newAllwiseMetadataService(t)
...
func TestMetadata_SomeDBError(t *testing.T) {
	m, _, db := newAllwiseMetadataService(t)
