...
func (a Adapter) BulkInsertMetadata(ctx context.Context, rows []any) error {
	...
	return repository.BulkInsert(ctx, a.repo, rows, func(ctx context.Context, qtx *repository.Queries, row repository.Erosita) error {
		return qtx.InsertErosita(ctx, repository.InsertErositaParams(row))
	})
}
